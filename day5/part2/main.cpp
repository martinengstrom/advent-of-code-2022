...
vector<stack<string>> get_stacks(vector<string> data) {
    ...
    for (long unsigned int i = 0; i < invertedStacks.size(); i++) {
...
// Fuck you dolphin.. ando fuck you CrateMover 9001
void execute_move(vector<stack<string>> *stacks, move_order order) {
    ...
    for (int i = temp.size() - 1; i >= 0; i--) {
        stacks->at(order.to - 1).push(temp[i]);
    }
}

void visualize_stacks(vector<stack<string>> stacks) {
    vector<stack<string>> copy = stacks;
    int i = 1;
    for (stack<string> s : copy) {
        cout << "Stack #" << i << endl;
        i++;
